...
def overlap_ratio(x1, y1, w1, h1, x2, y2, w2, h2):    # IoU 的计算，交集/并集
...
def evaluate_sliding_window(img_filename, crops):
    ...
    idx = np.argmax(scores[:len(crops)])  # 找到最大的scores的索引
...
def evaluate_FCDB():
    ...
    for item in sliding_windows:
        ...
        crops = item['crops']
        img_filename = join('FCDB', item['filename'])   # 拼接路径
        img = io.imread(img_filename)
        height = img.shape[0]
        width = img.shape[1]

        # ground truth，sliding_window.json的crops中的每第一个为ground truth
        x = crops[0][0]
        y = crops[0][1]
        w = crops[0][2]
        h = crops[0][3]
        ...
        if ratio >= alpha:       # 如果IoU值大于0.75，则alpha-recall加1
            alpha_cnt += 1
        accum_overlap_ratio += ratio
        cnt += 1
        crop_cnt += len(crops)

    print 'Average overlap ratio: {:.4f}'.format(accum_overlap_ratio / cnt)  # 平均IoU
